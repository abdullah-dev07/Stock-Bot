...
class CSharpLexer(RegexLexer):
    ...
    `unicodelevel`
      ...
      * ``full`` -- all Unicode characters as specified in the C
...
class CSharpAspxLexer(DelegatingLexer):
    ...
    def analyse_text(text):
        if re.search(r'Page\s*Language="C
            return 0.2
        elif re.search(r'script[^>]+language=["\']C
            return 0.15
...
class XppLexer(RegexLexer):
    ...
    tokens = {
        ...
        'class': [
            (XPP_CHARS, Name.Class, '
            default('
        ],
        'namespace': [
            (r'(?=\()', Text, '
            ('(' + XPP_CHARS + r'|\.)+', Name.Namespace, '
        ]
    }
