...
def _luau_make_expression(should_pop, _s):
    temp_list = [
        (r'0[xX][\da-fA-F_]*', Number.Hex, '
        (r'0[bB][\d_]*', Number.Bin, '
        (r'\.?\d[\d_]*(?:\.[\d_]*)?(?:[eE][+-]?[\d_]+)?', Number.Float, '
        ...
        (r'(\.)([a-zA-Z_]\w*)(?=%s*[({"\'])', bygroups(Punctuation, Name.Function), '
        (r'(\.)([a-zA-Z_]\w*)', bygroups(Punctuation, Name.Variable), '

        (rf'[a-zA-Z_]\w*(?:\.[a-zA-Z_]\w*)*(?={_s}*[({{"\'])', Name.Other, '
...
class JclLexer(RegexLexer):
    ...
    _JOB_HEADER_PATTERN = re.compile(r'^//[a-z
                                     re.IGNORECASE)
